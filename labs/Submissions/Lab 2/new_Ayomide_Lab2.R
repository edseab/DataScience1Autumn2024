...
prequel_trilogy <- matrix(prequel_trilogy, byrow = T, ncol = 2)
revenue <- rowSums(prequel_trilogy)
prequel_trilogy <- cbind(prequel_trilogy, revenue)
star_wars_matrix <- rbind(star_wars_matrix, prequel_trilogy)
rownames(star_wars_matrix)[4:6] <- c("The Phantom Menace", "Attack of clones", "Revenge of sith")
...
prob_k <- function(k, n, p) {
    prob <- p^k * (1 - p)^(n - k) * choose(n, k)
    return(prob)
}
...
# there is a 11 percent chance of returning 60% or less water signal. this 11 percent chance isnt rare enough to rule out the possibility the planet could be earth.
...
# since we are not particular about a specific direction, we do a 2-tailed test
