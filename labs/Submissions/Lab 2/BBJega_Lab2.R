...
total_sum <- rowSums(star_wars_matrix, na.rm = T)   
...
colnames(star_wars_matrix)[3] <- ("Total_revenue")
...
triology_matrix <- matrix(c(phantom_menace,attack_of_clones,revenge_of_sith),byrow = T, nrow=3)
star_wars_matrix <- rbind(star_wars_matrix,triology_matrix)
