...
colnames(star_wars_matrix)[3]<- c("Total revenue")
star_wars_matrix
...
trilogy_matrix<- matrix(trilogy, nrow=3, byrow = T)
rownames(trilogy_matrix)<- c("phantom_menace", "attack_of_clones","revenge_of_sith" )
Total_revenue<- rowSums(trilogy_matrix)
Total_revenue
trilogy_matrix<-cbind(trilogy_matrix, Total_revenue)
trilogy_matrix
...
simulations <- rbinom(100000, size = 20, prob = 0.7)
count_11_or_fewer <- sum(simulations <= 11)
count_11_or_fewer
# Calculation of the probability
probability <- count_11_or_fewer / 100000
probability
#If the planet were Earth, there’s an 11.27% chance of seeing such a result purely by chance
#This result isn’t common, but it’s also not extremely rare. Therefore, while it does cast 
#a bit of doubt, it doesn’t definitively rule out the possibility that this planet could be Earth.
...
men_heights <- c(1.75,1.65,1.95,1.80) ## Populate with the heights of men from class
women_heights <- c(1.52,1.60,1.54,1.64) ## Populate with the heights of women from class
...
#To capture the probability in both tails (both directions), we multiply this one-sided p-value pt() by 2