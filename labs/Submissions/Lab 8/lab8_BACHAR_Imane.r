...
replicate_results <- replicate(100, {
  p_values <- c()
  for (i in 1:60) {
      y <- rnorm(N)
      t_test <- t.test(y[treatment == 1], y[treatment == 0])
...
hist(replicate_results)

mean(replicate_results)
...
threshold <- 0.05 / 60
significant_results_ <- sum(p_values < threshold)
significant_results
...
model_toycars <- lm(distance ~ car, data = toycars)
summary(model_toycars)

# 3.4 Recode the 'car' variable so that car 1 is coded as 'green', car 2 is 'yellow', and car 3 is 'red'
toycars$car <- factor(toycars$car, labels = c("green", "yellow", "red"))
...
model_toycars_angle <- lm(distance ~ car + angle, data = toycars)
summary(model_toycars_angle)
# 3.7 What is the predicted distance traveled by a red car launched at 3 degrees?
predict(model_toycars_angle, newdata = data.frame(car = "red", angle = 3))

# 3.8 What is the predicted distance traveled by a white car launched at 2.5 degrees?
predict(model_toycars_angle, newdata = data.frame(car = "white", angle = 2.5))
...
ggplot(toycars, aes(x = angle, y = distance, color = car)) +
    ...
    labs(title = "Distance vs Angle by Car Type",
         x = "Angle (degrees)",
         y = "Distance (meters)")
