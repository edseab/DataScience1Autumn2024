...
# For barplots and boxplots, we buse barplot() and boxplot()
...
hist(mtcars$wt, 
  main = "Histogram of Car Weights in Kilograms", 
  xlab = "Weight (kg)", 
  col = "lightblue", 
  border = "black")

plot(density(mtcars$wt), 
  main = "Density Plot of Car Weights in Kilograms", 
  xlab = "Weight (kg)", 
  col = "darkblue", 
  lwd = 2)
...
barplot(barplot_d, beside = TRUE, ylim = c(0, 1), col = c("dodgerblue", "firebrick"), 
  main = "Proportion of Survivors by Class and Sex", 
  xlab = "Passenger Class", 
  ylab = "Proportion of Survivors")


legend(10,1, title = "sex", legend = rownames(barplot_d), fill = c("dodgerblue", "firebrick"), cex = 0.5)  


# 1.4. Looking at this barplot, what can you say about who was more likely to survive the Titanic?
#Females were more likely to survive than males across all classes
#The survival pattern reflects the "women and children first" protocol 
#that was followed during the evacuation of the Titanic.
...
plot(iris$Sepal.Length, iris$Petal.Length, 
  ...
  main = "Petal Length vs Sepal Length by Species", 
  xlab = "Sepal Length", 
  ylab = "Petal Length")

# regression line for Setosa
setosa <- iris[iris$Species == "setosa", ]
abline(lm(Petal.Length ~ Sepal.Length, data = setosa), col = 1)  

# regression line for Versicolor
versicolor <- iris[iris$Species == "versicolor", ]
abline(lm(Petal.Length ~ Sepal.Length, data = versicolor), col = 2) 
# regression line for Virginica
virginica <- iris[iris$Species == "virginica", ]
abline(lm(Petal.Length ~ Sepal.Length, data = virginica), col = 3) 
