...
mtcars_kg <- mtcars$wt * 0.453592
mtcars_kg
# 1.2 Plot a histogram and a density plot of the weights of cars in kg in the mtcars dataset.
hist(mtcars_kg,main ="Histogram of weight of cars in Kg" ,xlab="Weights of cars in Kg")
plot(density(mtcars_kg),main = "Density plot of the weights of cars in kg ",xlab="Weights of cars in Kg")
...
# Create the bar plot with customized colors, title, axis labels, and a legend
barplot(barplot_d, beside = TRUE, col = c("blue", "yellow"), 
        names.arg = c("1st", "2nd", "3rd", "Crew"), 
        main = "Survival Proportion by Sex and Class", 
        xlab = "Class", 
        ylab = "Proportion of Survivors",
        legend.text = c("Male", "Female"), 
        args.legend = list(x = "topright", bty = "n", fill = c("blue", "yellow")),
        border = "white")
...
# -Females were more likely to survive the Titanic than males especially the 1st class females.
# - The least likely group to survive was the males of the 2nd class.
...
plot(iris$Petal.Length, iris$Sepal.Length,
  main = "Relationship between Sepal and Petal Length in Irises",
  xlab = "Petal Length (cm)", ylab = "Sepal Length (cm)",
  pch = 20, col = iris$Species,  
  xlim = range(iris$Petal.Length), ylim = range(iris$Sepal.Length))
  legend("topleft", legend = levels(iris$Species), fill = c("red", "green", "blue"), 
  title = "Species", bty = "n",box.lwd = 8)
