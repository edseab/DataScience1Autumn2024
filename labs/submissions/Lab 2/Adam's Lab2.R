...
box_office2 <- c(phantom_menace, attack_of_clones, revenge_of_sith)
prequal_trilogy <- matrix(box_office2, byrow = TRUE, nrow = 3)
rownames(prequal_trilogy) <- c("The Phantom Menace", "Attack of the Clones", "Revenge of the Sith")
colnames(prequal_trilogy) <- c("US revenue", "International revenue")
prequal_trilogy
total_revenue2 <- rowSums(prequal_trilogy)
prequal_trilogy <- cbind(prequal_trilogy,total_revenue2)
colnames(prequal_trilogy)[3] <- "Total revenue"
prequal_trilogy
star_wars_matrix<- rbind(star_wars_matrix,prequal_trilogy)
star_wars_matrix
...
#This low percentage supports the astronomer’s hypothesis that the current observations (11 probes signaling water) are unlikely under the assumption that the planet has 70% water.
...
pnorm(195, 175, 10, lower.tail=FALSE)*100
...
men_heights <- c(174,171,185,190,165,181) ## Populate with the heights of men from class
women_heights <- c(160,171,165,159,169,158) ## Populate with the heights of women from class
