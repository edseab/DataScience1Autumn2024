...
new_box_office <- c(phantom_menace, attack_of_clones, revenge_of_sith)

star_wars_matrix


new_box_office<- matrix(new_box_office,ncol=2)
rownames(new_box_office)<- c("Phantom Menace", "Attack of Clones", " Revenge of Sith"  )

new_box_office

new_revenue <- rowSums(new_box_office)
new_box_office<- cbind(new_box_office, new_revenue)
colnames(new_box_office)<- NULL
new_box_office


star_wars_matrix<- rbind(star_wars_matrix,new_box_office)
star_wars_matrix
...
?factorial
probability_calculator <- function(n,k,p){
  proba = ((p^k)*((1-p)^(n-k))*choose(n,k))
  return(proba)
  }
probability_calculator(3,1,0.7)
...
probability_calculator(10,8,0.7)
...
probability_vector
for(i in 0:11){
  probability_vector=c(probability_vector,probability_calculator(20,i,0.7))
}
probability_vector
...
i= rbinom(100000,20,0.7)
total_probability<- sum(i<=11)
total_probability/100000

# What do you conclude to the astronomer?

""" Assuming an alpha value of 0.05, since the probability 0.11 is greater than 0.05, it means the difference is not statistically significant, we go with our hypothesis that this could be really be earth """
...
men_heights <- c(173, 179, 168, 179, 169, 181, 183, 167, 183, 170) ## Populate with the heights of men from class
women_heights <- c(157, 164, 158, 165, 156, 168, 162, 160, 165, 163) ## Populate with the heights of women from class
...
my_t <- function(x1,x2){
  ...
  df <- ((ss1 + ss2)^2)/(((ss1^2)/(n1-1))+ ((ss2^2)/(n2-1)))
...
#This is because its a 2-tailed test,  we are checking the statistics in both tails of the distribution
