...
#histogram plot
hist(wt_kg, main = "Weights of Cars in kilograms", xlab = "weight (in kg)", ylab = "frequency", breaks = 5)
#density plot
plot(density(wt_kg), main = "Weights of Cars in kilograms",
          xlab = "weight (in kg)", ylab = "frequency")
...
barplot(barplot_d, beside=T, ylim=c(0,1), col = c('midnightblue','pink'),
          xlab = 'Class', ylab = 'Proportion',
        main = 'Survival Rate of people on the Titanic')
legend('topright', legend = c("Male","Female"), bty = "n",
        fill = c('midnightblue','pink'))

# 1.4. Looking at this barplot, what can you say about who was more likely to survive the Titanic?
#I can say that females were more likely to survive the Titanic, because they had a higher survival rate in every group, the greatest being in the 1st class
...
plot(petal_length,sepal_length, xlab = 'Petal length (cm)',
        ylab = 'Sepal length (cm)', main = 'Relationship Between Petal and Sepal Length in Iris Species', pch = 18, col = species_colors)

legend("topleft", legend = c('Versicolor', 'Setosa', 'Virginica'),
         title = "Species", fill = c("yellow", "green", "orange"), inset = 0.05)

# Adding linear regression lines
abline(lm(Sepal.Length ~ Petal.Length, data = iris, subset = Species == "setosa"), col = "green")
abline(lm(Sepal.Length ~ Petal.Length, data = iris, subset = Species == "versicolor"), col = "yellow")
abline(lm(Sepal.Length ~ Petal.Length, data = iris, subset = Species == "virginica"), col = "orange")
