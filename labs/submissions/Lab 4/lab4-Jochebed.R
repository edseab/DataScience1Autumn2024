...
plot(dnorm(seq(-4,4,0.2)),type='o', col='#20cf1a')
...
barplot(barplot_d,beside=T, ylim=c(0,1), xlab='Class', ylab='Proportion of survivors', main='Proportion of survivors by sex and Class',col=c("#3c4bef","red1"), legend=c("Male","Female"))
...
#women or females(most especially first class) were more likely to survive
...
plot(iris$Petal.Length, iris$Sepal.Length, ylab= 'Sepal length(cm)', xlab='Petal length(cm)',main= "Relationship between petal and sepal length in irises", pch=18 )

# with species differentiated by color.

plot(iris$Petal.Length, iris$Sepal.Length, col= c(iris$Species),ylab= 'Sepal length(cm)', xlab='Petal length(cm)',main= "Relationship between petal and sepal length in irises", pch=18 )
