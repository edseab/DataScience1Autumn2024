...
~ dnorm(amir_height, height_mean,height_sd)
amir_percentile <- round(pnorm(amir_height,height_mean,height_sd) * 100)
amir_percentile

# OR

Z_score <- (amir_height - height_mean) / height_sd
round(pnorm(z_score)*100)
...
# The probability that none of the 10 selected indiviuals are eligible
